...
from transformers_1.pytorch_transformers.modeling_bert import (BertEmbeddings, 
...
class BertImgModel(BertPreTrainedModel):
    ...
    def getFeature(self, input_ids, token_type_ids=None, attention_mask=None,
            position_ids=None, head_mask=None, img_feats=None):
        if attention_mask is None:
            attention_mask = torch.ones_like(input_ids)
        ...
        # concatenate two embeddings
        cat_embedding_output = torch.cat((embedding_output, img_embedding_output), 1)

        encoder_outputs = self.encoder(cat_embedding_output,
                extended_attention_mask, head_mask=head_mask)
        sequence_output = encoder_outputs[0]
        pooled_output = self.pooler(sequence_output)

        return (embedding_output, img_embedding_output, pooled_output)
...
class ImageBertForSequenceClassification(BertPreTrainedModel):
    ...
    def getFeature(self, input_ids, token_type_ids=None, attention_mask=None, 
            position_ids=None, head_mask=None, img_feats=None):
        embedding_output, img_embedding_output, pooled_output = self.bert.getFeature(self, input_ids, token_type_ids, attention_mask,
            position_ids, head_mask, img_feats)
        drop_output = self.dropout(pooled_output)
        logits = self.classifier(drop_output)

        return (embedding_output, img_embedding_output, pooled_output, logits) # (Q, V, Z, A)

class OscarCX(nn.Module):
    def __init__(self):
        super(OscarCX, self).__init__()
        self.vqa_model = ImageBertForSequenceClassification.from_pretrained("./best")
        self.vqa_model.eval()
        ...
        self.linear_2 = nn.Linear(input_size, self.dim_h)
        self.linear_3 = nn.Linear(input_size, self.dim_h)
        self.out = nn.Linear(self.dim_h, 1)
        self.relu = nn.ReLU()
        self.drop = nn.Dropout(p=0.3)
    
    def vqa_forward(self, input_ids, token_type_ids=None, attention_mask=None, 
            position_ids=None, head_mask=None, img_feats=None):

        batch_size = img_feats.size(0)

		# Process all image features as a single batch
        v_emb = Variable(img_feats.view(
			batch_size * (self.knn_size + 1), -1))

        attention_mask_dup = Variable(attention_mask.view(
			batch_size * (self.knn_size + 1), -1))

        print("v_emb shape: ",v_emb.shape)
        print("attention_mask shaoe:", attention_mask_dup.shape)
        input_ids_dup = input_ids.view(batch_size, 1, -1).expand(batch_size,
                        self.knn_size + 1, -1).contiguous().view(batch_size * (self.knn_size + 1), -1)
        
        print("input_ids_dup shape: ",input_ids_dup.shape)
        token_type_ids_dup = token_type_ids.view(batch_size, 1, -1).expand(batch_size,
                        self.knn_size + 1, -1).contiguous().view(batch_size * (self.knn_size + 1), -1)
        
        position_ids_dup = position_ids.view(batch_size, 1, -1).expand(batch_size,
                        self.knn_size + 1, -1).contiguous().view(batch_size * (self.knn_size + 1), -1)
        head_mask_dup = head_mask.view(batch_size, 1, -1).expand(batch_size,
                        self.knn_size + 1, -1).contiguous().view(batch_size * (self.knn_size + 1), -1)
        q, v, z, a = self.vqa_model.getFeature(input_ids_dup, position_ids=position_ids_dup, token_type_ids=token_type_ids_dup,
                            attention_mask=attention_mask_dup, head_mask=head_mask_dup, img_feats=v_emb)
        ...
        return a_orig, z_orig, a_knns, z_knns, q, v

    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
            position_ids=None, head_mask=None, img_feats=None):

        batch_size = img_feats.size(0)
        a_orig, z_orig, a_knns, z_knns, q, v = self.vqa_forward(input_ids, position_ids=position_ids, token_type_ids=token_type_ids,
                            attention_mask=attention_mask, head_mask=head_mask, img_feats=img_feats)
        v_orig = Variable(v[:, 0], requires_grad=True).cuda()
        v_knns = Variable(v[:, 1:], requires_grad=True).cuda()
        ...
        for i in range(self.knn_size):
            ...
            input_feature = torch.cat((v_orig, v_other, v_mult, v_dist, v_rank,
                                q, z_orig, z_other, a_orig, a_other),
                                dim=1)
            h = self.drop(self.relu(self.linear_1(input_feature)))
